...
content: "## Intro\n\nI am starting to think through some patterns for replicating
  sqlite databases, exploring them, standardizing on schemas/models I use across projects,
  etc... Thanks to [Waylon](https://waylonwalker.com) I know about [[litestream]]
  and finally I started playing with it. Their [quickstart](https://litestream.io/getting-started/)
  is good, but it assumes you'll run MinIO on the same machine you run litestream
  on.\n\nI, however, have MinIO setup in my homelab with the api endpoint accessible
  via DNS - `s3.mydomain.com`.\n\nSo let's see what happens and then get to [the fix]\n\n\n```\n\u2B22
  ...
  secret-access-key: <SECRET-ACCESS-KEY>\n```\n\nSo with that config in place we can
  actually JUST `litestream replicate` from anywhere...\n\n```bash\n\u2B22 [devbox]
...
description: Intro I am starting to think through some patterns for replicating sqlite
  databases, exploring them, standardizing on schemas/models I use across projects,
  etc.
html:
  index: "<!DOCTYPE html>\n<html lang=\"en\">\n    <head>\n<title>Setup Remote MinIO
    ...
    patterns for replicating sqlite databases, exploring them, standardizing on schemas/models
    ...
    */\n    @media (max-width: 480px) {\n        .boundary-break-container {\n            height:
    ...
    through some patterns for replicating sqlite databases, exploring them, standardizing
    on schemas/models I use across projects, etc... Thanks to <a href=\"https://waylonwalker.com\">Waylon</a>
    I know about <a class=\"wikilink\" href=\"/litestream\">litestream</a> and finally
    I started playing with it. Their <a href=\"https://litestream.io/getting-started/\">quickstart</a>
    is good, but it assumes you'll run MinIO on the same machine you run litestream
    on.</p>\n<p>I, however, have MinIO setup in my homelab with the api endpoint accessible
    via DNS - <code>s3.mydomain.com</code>.</p>\n<p>So let's see what happens and
    then get to [the fix]</p>\n<pre class='wrapper'>\n\n<div class='copy-wrapper'>\n\n<button
  ...
  raw.md: "---\ndate: 2025-08-05 08:39:16\ntemplateKey: blog-post\ntitle: Setup Remote
    ...
    some patterns for replicating sqlite databases, exploring them, standardizing
    on schemas/models I use across projects, etc... Thanks to [Waylon](https://waylonwalker.com)
    I know about [[litestream]] and finally I started playing with it. Their [quickstart](https://litestream.io/getting-started/)
    is good, but it assumes you'll run MinIO on the same machine you run litestream
    on.\n\nI, however, have MinIO setup in my homelab with the api endpoint accessible
    via DNS - `s3.mydomain.com`.\n\nSo let's see what happens and then get to [the
    fix]\n\n\n```\n\u2B22 [devbox] \u276F litestream replicate quadtask-prod.db s3://s3.example.com/litestream/qt.db
    \   \n2025/08/05 08:50:09 INFO litestream version=v0.3.13\n2025/08/05 08:50:09
    ...
    \       # secret-access-key: <SECRET-ACCESS-KEY>\n```\n\nSo with that config in
    place we can actually JUST `litestream replicate` from anywhere...\n\n```bash\n\u2B22
